...
export default class ResearchController {
  static getResearch: RequestHandler = async (req, res, next) => {
    ...
    if (searchTitle === "") {
      r = await DB.getAllResearch({
        startFrom: startFrom,
        maxResults: perPage,
      });
    } else {
      r = await DB.searchResearchByTitle(searchTitle, {
        startFrom: startFrom,
        maxResults: perPage,
      });
    }
  ...
  static getResearchById: RequestHandler = async (req, res, next) => {
    ...
    if (n === null)
      throw new NotFoundError(
        `Research article with id ${req.params.id} does not exist.`,
      );
    res.json(n);
    next();
  };
  ...
  static createResearch: RequestHandler = async (req, res, next) => {
    ...
    try {
      const r = new ArticleIn(req.body).toArticle(

...
export default class MongoRepository implements IRepository {
  ...
  constructor(uri: string) {
    this.db = new MongoClient(uri, { serverApi: ServerApiVersion.v1 }).db(
      "AAPC",
    );
  ...
  async fetchMongoDocuments(
    ...
    if (options) {
      for await (const document of f
        .skip(options.startFrom ?? 0)
        .limit(options.maxResults ?? Infinity)) {
        a.push(document);
      }
    } else {
      for await (const document of f) {
  ...
  async getAllNews(
    ...
    const result = await this.fetchMongoDocuments(
      this.articles.find(q),
      options,
    );

...
export default class MongoRepository implements IRepository {
    ...
    async deleteResearch(id: string): Promise<void> {
        await this.articles.deleteOne({ id: id, articleType: ArticleType.research })
    }
    ...
    async getAllNews(options?: ArrayResultOptions<SortOptions<Article, ArticleSortFields>>): Promise<ArrayResult<Article>> {
        ...
        for (const document of result) { r.push(await this.documentToArticle(document)) }
        return {
            totalResults: rC,
            results: r
        }
    ...
    async getAllUsers(options?: ArrayResultOptions<SortOptions<User, UserSortFields>>): Promise<ArrayResult<User>> {
        ...
        for (const document of result) {r.push(new User(<object>document)) }
        return {
            totalResults: rC,
            results: r
        }
    ...
    async searchNewsByTitle(title: string, options?: ArrayResultOptions<SortOptions<Article, ArticleSortFields>>): Promise<ArrayResult<Article>> {
        const r: Article[] = []
        const q: Filter<any> = { articleType: ArticleType.news, title: { $regex: title } }
    ...
    async searchResearchByTitle(title: string, options?: ArrayResultOptions<SortOptions<Article, ArticleSortFields>>): Promise<ArrayResult<Article>> {
        const r: Article[] = []
        const q: Filter<any> = { articleType: ArticleType.research, title: { $regex: title } }
        const rC: number = await this.articles.countDocuments(q)
        const result = await this.fetchMongoDocuments(this.articles.find(q), options)
        for (const document of result) { r.push(await this.documentToArticle(document)) }
        return {

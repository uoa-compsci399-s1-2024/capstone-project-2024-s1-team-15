...
const LoginModal = forwardRef(({ modalId = "default-login-modal" }: LoginModalProps, ref: ForwardedRef<LoginModalRef>) => {
    ...
    useEffect(() => {
        ...
        s.addEventListener("click", () => {
            const modal = document.getElementById("inner-login-modal")
            if (!modal) return
    ...
    return (
        <div id={modalId} tabIndex={-1} className={`${hidden? "hidden" : ""} outer-modal`}>
            <div id={"inner-login-modal"} className={"inner-modal"}>
                <div className={"relative flex flex-row bg-white p-6 pb-4 rounded-t-2xl items-center"}>
                    <p className={"text-2xl font-bold grow ml-2"}>Login</p>
